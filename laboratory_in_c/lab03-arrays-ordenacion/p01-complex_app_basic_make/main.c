...
int main(int argc, char **argv) {
    ...
    if (*end != '\0') { fprintf(stderr, "Error: re1 no válido\n"); return 1; }

    double im1 = strtod(argv[3], &end);
    if (*end != '\0') { fprintf(stderr, "Error: im1 no válido\n"); return 1; }

    double re2 = strtod(argv[4], &end);
    if (*end != '\0') { fprintf(stderr, "Error: re2 no válido\n"); return 1; }

    double im2 = strtod(argv[5], &end);
    if (*end != '\0') { fprintf(stderr, "Error: im2 no válido\n"); return 1; }
